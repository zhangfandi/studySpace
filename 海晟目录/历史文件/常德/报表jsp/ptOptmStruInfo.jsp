...
/*	var spotOrgCd = "";
	...
	function setSQL(){
		...
		tableTitle = "单位,户数,完成户数,完成户数占比,合同面积(亩),优化结构面积(亩),完成比例%,0片,1-2片,3-4片,5-6片,小于0.45M,0片,1-2片,3-4片,较密,正常,较稀,开始日期,结束日期";
		...
		sql += "     d.HTMJ,d.OPTM_AREA,0 as wcbl,";
		sql += "     A.bottom1,A.bottom2,A.bottom3,A.bottom4,A.top1,A.top2,A.top3,A.JM,A.ZC,A.JS,";
		...
		sql += "      sum(case when GIVE_UP_BOTTOM='1' then OPTM_AREA end) as bottom1,";
		sql += "      sum(case when GIVE_UP_BOTTOM='2' then OPTM_AREA end) as bottom2,";
		sql += "      sum(case when GIVE_UP_BOTTOM='3' then OPTM_AREA end) as bottom3,";
		sql += "      sum(case when GIVE_UP_BOTTOM='3' then OPTM_AREA end) as bottom4,";
		sql += "      sum(case when GIVE_UP_TOP='1' then OPTM_AREA end) as top1,";
		sql += "      sum(case when GIVE_UP_TOP='2' then OPTM_AREA end) as top2,";
		sql += "      sum(case when GIVE_UP_TOP='3' then OPTM_AREA end) as top3,";
		sql += "      sum(case when WORK_EFFECT_EVAL='1' then OPTM_AREA end) as jm,";
		sql += "      sum(case when WORK_EFFECT_EVAL='2' then OPTM_AREA end) as zc,";
		sql += "      sum(case when WORK_EFFECT_EVAL='3' then OPTM_AREA end) as js,";
		sql += "      min(BEGIN_DATE) as BEGIN_DATE,";
		sql += "      max(END_DATE) as END_DATE";
		sql +=" FROM V_PT_OPTM_STRU ";
		sql += whereSql;
		sql += groupBySql;
		sql += " ) A";
		sql += " LEFT JOIN ";
		sql += " (SELECT A.ORG_CD,";
		sql += "     SUM(A.OPTM_AREA) AS OPTM_AREA,";     //优化结构面积
		sql += "     SUM(B.HTMJ) AS HTMJ,";      //合同面积
		sql += "     COUNT(DISTINCT A.FARMER_CD) AS hs,";  //户数
		sql += "     COUNT(DISTINCT (case when A.OPTM_AREA = B.HTMJ THEN A.FARMER_CD END)) AS wchs "; //完成户数
		sql += " FROM ";
		sql +=" (SELECT "+groupOrgCdField+" AS ORG_CD,";
		//sql += "  		MAX(A."+groupOrgNameField+") AS ORG_NAME, ";
		sql += "      FARMER_CD,";
		sql += "   SUM(OPTM_AREA) as OPTM_AREA";  //优化结构面积
		sql +=" FROM V_PT_OPTM_STRU ";
		sql +=whereSql;
       ...
       /* sql += "   SELECT  FARMER_CD,";
        sql += "   SUM(TRANSPLANT_AREA) as TRANSPLANT_AREA";  //移栽面积
