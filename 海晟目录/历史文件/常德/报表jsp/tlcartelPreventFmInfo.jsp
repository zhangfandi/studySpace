...
//        preNext = ['1,2,3,4,5,6,7'];
    ...
    function setSQL() {
        ...
        tableTitle = "单位,户数,植保面积(亩),专业化服务比例%,好,较好,一般,差,开始日期,结束日期";

        sql = "SELECT serv.SERV_ORG_NAME,COUNT(DISTINCT a.FARMER_CD) AS hs,SUM(a.PREVENT_AREA) AS PREVENT_AREA, ";
        sql += " cast(sum(CASE WHEN IS_PROPESS = '1' THEN PREVENT_AREA END) as double)*100/sum(CASE WHEN IS_PROPESS = '1' OR IS_PROPESS='0' THEN PREVENT_AREA END) AS zyhbl, ";
        sql += " SUM(CASE WHEN a.PREVENT_RESULT = '01' THEN PREVENT_AREA END) AS hao,";
        sql += " SUM(CASE WHEN a.PREVENT_RESULT = '02' THEN PREVENT_AREA END) AS jhao,";
        sql += " SUM(CASE WHEN a.PREVENT_RESULT = '03' THEN PREVENT_AREA END) AS yban,";
        sql += " SUM(CASE WHEN a.PREVENT_RESULT = '04' THEN PREVENT_AREA END) AS cha,";
        sql += " substr(CHAR(TIMESTAMP(min(a.BUSINESS_DATE))),1,10) AS BEGIN_DATE,";
        sql += " substr(CHAR(TIMESTAMP(MAX(a.BUSINESS_DATE))),1,10) AS END_DATE";
        sql += " FROM V_PT_PREVENT a ";
