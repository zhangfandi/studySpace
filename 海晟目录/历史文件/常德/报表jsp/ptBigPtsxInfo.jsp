...
/*	g_pageRow = -1;
	...
	function initParam(){
		...
		unitType="0,4,4,0,1,1,0,1,1,1,1,1,1,0,0";
		//小数位
		dotCol ="0,0,0,2,2,2,2,2,2,2,2,2,2,0,0";
		sumForm = "4=%3/2,7=%6/5";
		showNoRow = false;
	}
...
/*	var spotOrgCd = "";
	...
	function setSQL(){
        ...
        tableTitle = "单位,户数,完成户数,完成户数占比,合同面积(亩),培土面积(亩),完成比例%,垄高高度达标(亩),垄高高度未达标(亩),好,较好,一般,差,开始日期,结束日期";
        ...
        sql = "";
        sql += " SELECT A.ORG_NAME, D.hs,D.wchs, 0 as wchsbl,";
        sql += "     D.HTMJ,D.PTSX_AREA,0 as wcbl,";
        ...
        sql += "      sum(case when IS_HIGH_STAND='1' then PTSX_AREA end) as qldb,";  //垄高高度达标
        sql += "      sum(case when IS_HIGH_STAND='0' then PTSX_AREA end) as qlwdb,";  //垄高高度未达标
        sql += "      sum(case when WORK_EFFECT_EVAL='01' then PTSX_AREA end) as hao,";
        sql += "      sum(case when WORK_EFFECT_EVAL='02' then PTSX_AREA end) as jhao,";
        sql += "      sum(case when WORK_EFFECT_EVAL='03' then PTSX_AREA end) as yban,";
        sql += "      sum(case when WORK_EFFECT_EVAL='04' then PTSX_AREA end) as cha,";
       /* sql += "       TO_CHAR(TIMESTAMP(min(BEGIN_DATE)),'YYYY-MM-DD') as BEGIN_DATE,";
        ...
        sql +=" FROM V_PT_BIG_PTSX ";
        sql += whereSql;
        sql += groupBySql;
        sql += " ) A";
        sql += " LEFT JOIN ";
        sql += " (SELECT A.ORG_CD,";
        sql += "     SUM(A.PTSX_AREA) AS PTSX_AREA,";     //培土面积
        sql += "     SUM(B.HTMJ) AS HTMJ,";                 //种植批复面积
        sql += "     COUNT(DISTINCT A.FARMER_CD) AS hs,";  //户数
        sql += "     COUNT(DISTINCT (case when A.PTSX_AREA = B.HTMJ THEN A.FARMER_CD END)) AS wchs "; //起垄面积等于批复面积的烟农为完成户数
        sql += " FROM ";
        sql +=" (SELECT "+groupOrgCdField+" AS ORG_CD,";
        sql += "      FARMER_CD,";
        sql += "   SUM(PTSX_AREA) as PTSX_AREA";  //培土面积
        sql +=" FROM V_PT_BIG_PTSX ";
        sql +=whereSql;
