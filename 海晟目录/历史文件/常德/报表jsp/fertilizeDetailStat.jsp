...
//        ];
        ...
        tableTitle = "乡镇,村,烟农编码,烟农姓名,合同面积(亩),施肥面积(亩),完成比例,是否按套餐施肥,施肥标准,开始日期,结束日期,烟技员";
        ...
        sql += "   A.FERTILIZ_AREA,";
        sql += getSqlDivisionExpression("A.FERTILIZ_AREA","B.CNTRCT_PLNT_AREA",true,2)+",";
        sql += "   CASE WHEN A.IS_MEAL = '1'";
        sql += "     THEN '是'";
        sql += "   WHEN A.IS_MEAL = '0'";
        sql += "     THEN '否'";
        sql += "   END AS IS_MEAL,";
        sql += "   CASE WHEN A.FERTILIZ_STAND = '1'";
        sql += "     THEN '正常'";
        sql += "   WHEN A.FERTILIZ_STAND = '2'";
        sql += "     THEN '偏高'";
        sql += "   WHEN A.FERTILIZ_STAND = '3'";
        sql += "     THEN '偏低'";
        sql += "   END AS FERTILIZ_STAND,";
        sql += ts2cd("A.BEGIN_DATE")+",";
        sql += ts2cd("A.END_DATE")+",";
        sql += "   C.TECHNICIAN_NAME";
        sql += " FROM";
        sql += "   V_PT_PUT_FERTILIZ A";
        sql += "   LEFT JOIN";
        sql += "   (";
        sql += "     SELECT";
        sql += "       FARMER_CD,";
