...
<script>
    ...
    function setSQL(){
        ...
        tableTitle = "单位,户数,完成户数,完成户数占比,合同面积(亩),覆膜面积(亩),完成比例%,已覆膜（亩）,未覆膜（亩）,严实,不严实,好,较好,一般,差,开始日期,结束日期";
        sql = " SELECT";
        sql += "   ORG.ORG_NAME,";
        ...
        sql += "   A.VELUM_AREA,";
        sql += getSqlDivisionExpression("A.VELUM_AREA","BC.CNTRCT_PLNT_AREA",true,2)+",";
        sql += "   VELUM_T,";
        sql += "   VELUM_F,";
        sql += "   EFFTET_T,";
        sql += "   EFFTET_F,";
        ...
        sql += "           WHEN VELUM_EFFTET = '1' OR VELUM_EFFTET = '2'";
        sql += "             THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS VELUM_T,";
        sql += "       SUM(CASE WHEN VELUM_EFFTET <> '1' AND VELUM_EFFTET <> '2'";
        sql += "         THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS VELUM_F,";
        sql += "       SUM(CASE";
        sql += "           WHEN VELUM_EFFTET = '1'";
        sql += "             THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS EFFTET_T,";
        sql += "       SUM(CASE";
        sql += "           WHEN VELUM_EFFTET = '2'";
        sql += "             THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS EFFTET_F,";
        sql += "       SUM(CASE";
        sql += "           WHEN WORK_EFFECT_EVAL = '01'";
        sql += "             THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS EVAL_1,";
        sql += "       SUM(CASE";
        sql += "           WHEN WORK_EFFECT_EVAL = '02'";
        sql += "             THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS EVAL_2,";
        sql += "       SUM(CASE";
        sql += "           WHEN WORK_EFFECT_EVAL = '03'";
        sql += "             THEN VELUM_AREA";
        sql += "           ELSE 0 END) AS EVAL_3,";
        sql += "       SUM(CASE";
        sql += "           WHEN WORK_EFFECT_EVAL = '04'";
        sql += "             THEN VELUM_AREA";
        ...
        sql += "       SUM(B.VELUM_AREA)           AS VELUM_AREA,";
        sql += "       SUM(C.CNTRCT_PLNT_AREA)     AS CNTRCT_PLNT_AREA,";
        sql += "       COUNT(DISTINCT (";
        sql += "         CASE";
        sql += "         WHEN B.VELUM_AREA = C.CNTRCT_PLNT_AREA";
