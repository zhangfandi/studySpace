...
/*	var spotOrgCd = "";
	...
	function setSQL(){
		...
		tableTitle = "乡镇,村,烟农编码,烟农姓名,移栽面积(亩),优化结构面积(亩),完成比例%,打掉下部,弃采上部,实施效果评价,开始日期,结束日期,烟技员";
		...
		sql=" SELECT A.TOWN_NAME,A.VAGE_NAME,A.FARMER_CD,A.FARMER_NAME,B.HTMJ,A.OPTM_AREA,0 AS WCBL,";
		sql+="   X1.ENUM_NAME AS GIVE_UP_BOTTOM ,";
		sql+="   X2.ENUM_NAME AS GIVE_UP_TOP,";
		sql+="   X3.ENUM_NAME AS WORK_EFFECT_EVAL,";
		sql+="   A.BEGIN_DATE,";
        sql+="   A.END_DATE,";
        sql+= "  C.TECHNICIAN_NAME";
		sql+=" FROM";
		sql+=" (SELECT TOWN_NAME,VAGE_NAME,FARMER_CD,FARMER_NAME,GIVE_UP_BOTTOM,GIVE_UP_TOP,WORK_EFFECT_EVAL,";
        /*sql+=" TO_CHAR(TIMESTAMP(BEGIN_DATE),'YYYY-MM-DD') as BEGIN_DATE ,";
        sql+=" TO_CHAR(TIMESTAMP(END_DATE),'YYYY-MM-DD') as END_DATE,";*/
        sql+=" BEGIN_DATE ,";
        sql+="  END_DATE,";
        sql+=" SUM(OPTM_AREA) AS OPTM_AREA";
		sql+=" FROM V_PT_OPTM_STRU";
		sql+=whereSql;
		sql+=" GROUP BY TOWN_NAME,VAGE_NAME,FARMER_CD,FARMER_NAME,GIVE_UP_BOTTOM,GIVE_UP_TOP,WORK_EFFECT_EVAL,";
       // sql+="       TO_CHAR(TIMESTAMP(BEGIN_DATE),'YYYY-MM-DD'),TO_CHAR(TIMESTAMP(END_DATE),'YYYY-MM-DD')) A";
        sql+=" BEGIN_DATE,END_DATE) A";
		sql+=" LEFT JOIN (";
		...
		sql+=" LEFT JOIN pt_ix_enum_data X1 ON X1.ENUM_CD=A.GIVE_UP_BOTTOM AND X1.DATA_STATE='1' AND X1.idx_id='8a3c34536a8cad24016a950c12d2004a' ";  //打掉下部
		sql+=" LEFT JOIN pt_ix_enum_data X2 ON X2.ENUM_CD=A.GIVE_UP_TOP AND X2.DATA_STATE='1' AND X2.idx_id='8a3c34536a8cad24016a950ccb18004f' ";   //弃采上部
